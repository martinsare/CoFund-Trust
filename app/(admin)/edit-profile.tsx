    ...
    >
      ...
      >
        <Animated.View entering={FadeInDown.delay(0).duration(400)} style={styles.headerRow}>
          ...
          <Text style={[styles.headerTitle, { color: colors.foreground }]}>Edit Admin Profile</Text>
          <PressableScale
            onPress={handleSave}
            disabled={!hasChanges || saving}
            style={[styles.saveBtn, { backgroundColor: hasChanges && !saving ? "#7c3aed" : colors.muted }]}
          >
        ...
        <Animated.View entering={FadeInUp.delay(80).duration(400)} style={styles.avatarSection}>
          <Pressable onPress={handleAvatarPress} style={styles.avatarPressable}>
            ...
            <View style={[styles.avatarEditBadge, { backgroundColor: "#7c3aed", borderColor: colors.background }]}>
          ...
          {showCountryPicker && (
            <View style={[styles.countryList, { borderTopColor: colors.borderLight }]}>
              {COUNTRIES.map((c) => (
                <Pressable
                  key={c}
                  style={[styles.countryItem, { borderBottomColor: colors.borderLight }]}
                  onPress={() => { setCountry(c); setShowCountryPicker(false); }}
                >
                  <Text style={[styles.countryItemText, { color: c === country ? "#7c3aed" : colors.foreground }]}>{c}</Text>
                  {c === country && <Feather name="check" size={14} color="#7c3aed" />}
                </Pressable>
              ))}
            </View>
          )}
          ...
          <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <View style={styles.fieldRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.fieldValue, { color: colors.foreground }]}>Super Admin</Text>
              </View>
            </View>
          </View>
        </Animated.View>
        ...
        <Animated.View entering={FadeIn.delay(340).duration(400)}>
          <PressableScale
            onPress={handleSave}
            disabled={!hasChanges || saving}
            style={[styles.saveBottomBtn, { backgroundColor: hasChanges && !saving ? "#7c3aed" : colors.muted }]}
          >
